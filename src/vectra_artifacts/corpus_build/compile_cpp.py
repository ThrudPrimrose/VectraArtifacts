...
def compile_object(src, build_dir, extra_flags: Optional[List[str]] = None, force: bool = False) -> pathlib.Path:
    ...
    subprocess.check_call([CXX, "-c"] + flags + [str(src), "-o", str(obj)])
    return obj
...
def compile_cpp_library(
    ...
    if jobs > 1:
        ...
        with ThreadPoolExecutor(max_workers=jobs) as pool:
            futs = {pool.submit(compile_object, cpp, build_dir, extra_flags, force): cpp for cpp in cpps}
...
def main_compile_cpp(default_root: str,
    ...
    if args.jobs > 1:
        ...
        with ThreadPoolExecutor(max_workers=args.jobs) as pool:
            futs = {pool.submit(compile_object, cpp, args.build_dir, force=args.force): cpp for cpp in cpps}
            for fut in as_completed(futs):
                cpp = futs[fut]
                try:
                    obj = fut.result()
        ...
        objects = [compile_object(cpp, args.build_dir, force=args.force) for cpp in cpps]
