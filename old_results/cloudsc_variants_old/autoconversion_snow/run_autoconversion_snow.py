...
KLON, NCLDQS, NCLDQI = 4194304, 4, 2
RTOL = 1e-9
...
def make_inputs():
    ...
    consts = dict(
        rtt=273.16, rlcritsnow=1e-4, rsnowlin1=1e-3, rsnowlin2=0.05,
        rnice=1e7, ptsphy=2.0, zepsec=1e-12, laericeauto=1,
    )
    arrays = dict(
...
def numpy_reference(consts, arrays):
    import importlib.util
    spec = importlib.util.spec_from_file_location(
        "ref", os.path.join(HERE, "autoconversion_snow_numpy.py"))
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    a = {k: v.copy() for k, v in arrays.items()}
    mod.autoconversion_snow(a["ztp1"], a["zicecld"], a["pnice"], a["zsolqb"],
                            a["zsnowaut"], consts["rtt"], consts["rlcritsnow"],
                            consts["rsnowlin1"], consts["rsnowlin2"], consts["rnice"],
                            consts["ptsphy"], consts["zepsec"], consts["laericeauto"],
                            KLON, NCLDQS, NCLDQI)
    return {"zsnowaut": a["zsnowaut"], "zsolqb": a["zsolqb"]}


# --------------------------------------------------------------------------
# original Fortran (.so via ctypes), bind-C with timer
# --------------------------------------------------------------------------
def run_original_fortran(consts, arrays):
    so = os.path.join(HERE, "libautoconversion_snow_orig.so")
    if not os.path.exists(so):
        subprocess.run(["gfortran", "-O3", "-fPIC", "-shared", "-ffast-math",
                        "-fno-math-errno",
                        os.path.join(HERE, "autoconversion_snow_w_timer.f90"),
                        "-o", so], check=True)
    lib = ctypes.CDLL(so)
    ...
    timer = np.zeros(2)
    fn(1, KLON, KLON, ztp1, zicecld, pnice, zsolqb, zsnowaut,
       consts["rtt"], consts["rlcritsnow"], consts["rsnowlin1"], consts["rsnowlin2"],
       consts["rnice"], consts["ptsphy"], consts["zepsec"], bool(consts["laericeauto"]),
       NCLDQS, NCLDQI, timer)
...
# --------------------------------------------------------------------------
def run_native(consts, arrays, lang_map, lang):
...
# --------------------------------------------------------------------------
def run_sdfg(tag, consts, arrays, fortran_layout):
    import dace
    sdfg = dace.SDFG.from_file(os.path.join(HERE, f"autoconversion_snow_{tag}.sdfg"))
    csdfg = sdfg.compile()
    # DaCe rejects array *views*; every buffer must be a fresh owning copy.
    order = "F" if fortran_layout else "C"
    zsolqb = np.array(arrays["zsolqb"], order=order, copy=True)
    zsnowaut = np.array(arrays["zsnowaut"], order=order, copy=True)
    ztp1 = np.array(arrays["ztp1"], order=order, copy=True)
    zicecld = np.array(arrays["zicecld"], order=order, copy=True)
    pnice = np.array(arrays["pnice"], order=order, copy=True)
    sc = {k: np.float64(consts[k]) for k in
          ("rtt", "rlcritsnow", "rsnowlin1", "rsnowlin2", "rnice", "ptsphy", "zepsec")}
    if tag == "fortran_frontend":
        csdfg(pnice=pnice, zicecld=zicecld, zsnowaut=zsnowaut, zsolqb=zsolqb, ztp1=ztp1,
              laericeauto=np.bool_(consts["laericeauto"] != 0),
              kfdia=KLON, klon=KLON, ncldqi=NCLDQI, ncldqs=NCLDQS, **sc)
    else:
        csdfg(pnice=pnice, zicecld=zicecld, zsnowaut=zsnowaut, zsolqb=zsolqb, ztp1=ztp1,
              laericeauto=np.int64(consts["laericeauto"]),
              KLON=KLON, NCLDQI=NCLDQI, NCLDQS=NCLDQS, **sc)
    return {"zsnowaut": zsnowaut, "zsolqb": zsolqb}


def main():
    consts, arrays = make_inputs()
    ref = numpy_reference(consts, arrays)
