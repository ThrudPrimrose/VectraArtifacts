...
echo "uenv                          gcc_version"
echo "-----------------------------  -----------"

for u in "${UENVS[@]}"; do
  version=$(uenv run "$u" --view=default -- gcc --version 2>/dev/null | head -1)

  if [ -z "$version" ]; then
    # fallback: some images may not accept --view=default, retry without it
    version=$(uenv run "$u" -- gcc --version 2>/dev/null | head -1)
  fi

  if [ -z "$version" ]; then
    version="ERROR: could not start / gcc not found"
  fi
