...
CONST_ORDER = [
    "rtt", "rv", "rd", "rprecrhmax", "rcovpmin", "rdensref", "ptsphy", "zepsec", "rcl_fac1", "rcl_fac2", "rcl_cdenom1",
    "rcl_cdenom2", "rcl_cdenom3", "rcl_ka273", "rcl_const1r", "rcl_const2r", "rcl_const3r", "rcl_const4r"
]
...
def numpy_reference(arrays):
    ...
    mod.rain_evaporation_abel_boutle(a["ztp1"], a["zqx_ncldqv"], a["za"], a["zqsliq"], a["zqxfg_ncldqr"], a["zcovptot"],
...
def run_original_fortran(arrays, timer_out=None):
    ...
    fn.argtypes = ([ci, ci, ci] + [nd1] * 5 + [nd1] * 4 + [nd1] + [nd3, nd1] + [cd] * 18 + [ci, ci, ci, nd1])
    a = {k: np.asfortranarray(v.copy()) for k, v in arrays.items()}
    timer = timer_out if timer_out is not None else np.zeros(2)
    fn(1, KLON, KLON, a["ztp1"], a["zqx_ncldqv"], a["za"], a["zqsliq"], a["zqxfg_ncldqr"], a["zcovptot"], a["zcovpclr"],
       a["zcovpmax"], a["zrho"], a["pap"], a["zsolqa"], a["zevap_out"], *[CONSTS[k] for k in CONST_ORDER], NCLDQV,
       NCLDQR, NCLV, timer)
    return {k: a[k] for k in OUT}
...
# rain_evaporation_abel_boutle's python-frontend SDFG declares its one
# multi-dimensional array (zsolqa) axis-reversed so KLON lands at unit stride
# under DaCe's default C-contiguous storage -- see
# rain_evaporation_abel_boutle_dace.py's module docstring and
# lu_solver_dace.py's for the full rationale. ``.T`` is a full-axis reversal
# for any ndim and a safe no-op for 1-D arrays, so it applies unconditionally
# to every array here without needing to single out which ones are affected.
