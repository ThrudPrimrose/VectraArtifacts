...
def numpy_reference(arrays):
    ...
    mod.compute_saturation_values(a["ztp1"], a["pap"], a["zfoealfa"], a["zfoeewmt"], a["zqsmix"], a["zfoeew"],
                                  a["zqsice"], a["zfoeeliqt"], a["zqsliq"], *[CONSTS[k] for k in CONST_ORDER], KLON,
                                  KLEV)
...
def run_original_fortran(arrays, timer_out=None):
    ...
    fn(1, KLON, KLON, KLEV, a["ztp1"], a["pap"], a["zfoealfa"], a["zfoeewmt"], a["zqsmix"], a["zfoeew"], a["zqsice"],
       a["zfoeeliqt"], a["zqsliq"], *[CONSTS[k] for k in CONST_ORDER], timer)
    return {k: a[k] for k in OUT}
...
def sdfg_kwargs(tag, a):
    ...
    if tag == "fortran_frontend":
        return dict(**a, **sc, kidia=1, kfdia=KLON, klon=KLON, klev=KLEV)
    return dict(**a, **sc, KLON=KLON, KLEV=KLEV)
