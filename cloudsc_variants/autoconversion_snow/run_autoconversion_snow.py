...
# --------------------------------------------------------------------------
# autoconversion_snow's python-frontend SDFG declares its one multi-dimensional
# array (zsolqb) axis-reversed so KLON lands at unit stride under DaCe's default
# C-contiguous storage -- see autoconversion_snow_dace.py's module docstring and
# lu_solver_dace.py's for the full rationale. ``.T`` is a full-axis reversal for
# any ndim and a safe no-op for 1-D arrays, so it applies unconditionally to
# every array here without needing to single out which ones are affected.
